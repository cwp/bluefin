export default class Database {
  ...
  async hasCitus() {
    if (!('_hasCitus' in this)) {
      this._hasCitus = this.getHasCitus()
    }
    return this._hasCitus
  }
  ...
  async getHasCitus() {
    const client = await this.connect()
    return client.value("select exists (select 1 from pg_extension where extname = 'citus')")
  }
}
