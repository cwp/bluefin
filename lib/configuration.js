...
import _transactions from './middleware/transactions'
import Path from './path'
import Plan from './plan'
import {Destruction, Grant, Migration, Program, Revocation, Role, Table} from './program'

const allMiddleware = {transactions: _transactions}
...
class DatabaseConfiguration {
  ...
  dsn(dbName = this.name) {
    const dsn = {database: dbName, application_name: 'bluefin', keepAlive: true}
  ...
  async loadMiddleware() {
    ...
    if ('middleware' in this.raw) {
      for (let name of this.raw.middleware) {
        modules.push(allMiddleware[name])
      }
    }
  ...
  createEnvironment() {
    const env = merge.all([defaultEnv, this.raw.env || {}, {database: this.name}])

    env.roleNames = []
    if (this.raw.grants instanceof Object) {
      for (let name of Object.keys(this.raw.grants)) {
        env.roleNames.push(name)
      }
    }
    env.has.roles = env.roleNames.length > 0
  ...
  async migrationPhase(options) {
    if ('migrations' in options && !options.migrations) return []
